...
def s_premium_dov(params, substep, state_history, previous_state, policy_input):
    premium = previous_state['init_eth_price'] * previous_state['options_sold'] * params['premium_rate']
...
def s_fee_dov(params, substep, state_history, previous_state, policy_input):
    ...
    return 'fee_dov', fee_dov if not previous_state['expired'] else 0

def s_options_sold(params, substep, state_history, previous_state, policy_input):
    options_sold = math.floor(previous_state['dov_quantity'] * 0.90)
...
def s_dummy(params, substep, state_history, previous_state, policy_input):
    return 'dummy', True
